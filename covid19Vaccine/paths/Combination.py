...
def LiveGeneticAlgorithmPlot(population, popSize, elitePercent, mutationRate, generations):
    eliteSize = int(round(elitePercent*popSize))
    pop = initialPopulation(popSize, population)
    progress = []
    progress.append(1 / rankRoutes(pop)[0][1])
...
def WebGA(popSize, elitePercent, mutationRate, generations):
    population = countries = [['Algeria', 2.9999825, 28.0000272], ['Nigeria', 7.9999721, 9.6000359], ['Senegal', -14.4529612, 14.4750607],
    ...
    bestRouteIndex = rankRoutes(pop)[0][0]
    bestRoute = pop[bestRouteIndex]
    return [ {'lat': parts.lat,'lon' : parts.lon} for parts in bestRoute]
